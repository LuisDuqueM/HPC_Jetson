...
#include <iomanip>
#include <sstream>
#include <fstream>   // para escribir CSV y leer /proc/meminfo
...
void reduce_sum_kernel(const float* data,
    ...
    // El hilo 0 del bloque escribe la suma parcial
    if (tid == 0) {
...
int main() {
    ...
    fout << "N,"
        "cpu_sum_ms,cpu_red_ms,"
        "h2d_ms,gpu_sum_kernel_ms,gpu_red_kernel_ms,d2h_ms,total_gpu_ms,"
        "speedup_sum_kernel,speedup_red_kernel,speedup_total_gpu\n";
    ...
    std::vector<size_t> tamaños = {
        ...
        1000000ul,      // 1e6
        10000000ul,     // 1e7
        100000000ul     // 1e8 (ajusta si no te alcanza la RAM o VRAM)
    };
